...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    # rays: 3 dim origin, 3dim dir, 2 dim near and far, 3 dim viewdirs
    # Render and reshape
...
def create_nerf(args):
    ...
    """
    # get embed function and input channel (to MLP) of (x, y, z)
    embed_fn, input_ch = get_embedder(args.multires, args.i_embed)

    input_ch_views = 0
    embeddirs_fn = None
    # get embed function and input channel (to MLP) of (phi, theta) direction
    ...
    # this is for hierarchical fine model
    model_fine = None
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    """
    # (1-exp(-sigma * delta)) delta -> distance
    raw2alpha = lambda raw, dists, act_fn=F.relu: 1.-torch.exp(-act_fn(raw)*dists)

    dists = z_vals[...,1:] - z_vals[...,:-1]
    # set the infinite alpha to be 1
    ...
    # in NeRV, this is expected depth
    depth_map = torch.sum(weights * z_vals, -1)
    disp_map = 1./torch.max(1e-10 * torch.ones_like(depth_map), depth_map / torch.sum(weights, -1))
    acc_map = torch.sum(weights, -1)

    if white_bkgd:
        # acc_map = rgb_map when rgb=1
...
def render_rays(ray_batch,
    ...
    # prime linear sampling along the ray
    ...
    if perturb > 0.:
        ...
        # get the medium point between samples
        mids = .5 * (z_vals[...,1:] + z_vals[...,:-1])
        # get the upper and lower bounds of each sample interval
        ...
        # random sample along the ray
        z_vals = lower + (upper - lower) * t_rand

    # get input points N * 3
...
#     raw = run_network(pts)
    ...
    if N_importance > 0:
        # use network_fine for the fine sampled points
...
#         raw = run_network(pts, fn=run_fn)
    ...
    # this is the coarse network output
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def config_parser():
    ...
    # add some jitter to the input
    parser.add_argument("--perturb", type=float, default=1.,
                        help='set to 0. for no jitter, 1. for jitter')

    # 5D (x, y, z, phi, theta) 3D (x, y, z)
    parser.add_argument("--use_viewdirs", action='store_true', 
                        help='use full 5D input instead of 3D')
    # position encoding : fourier feature
    ...
    # down sampling factor on image
    parser.add_argument("--render_factor", type=int, default=0, 
                        help='downsampling factor to speed up rendering, set 4 or 8 for fast preview')

    # training options
    # central crop method for training
...
def train():
    ...
    # config settings through argumentparser
    ...
    # save the args in args.txt
    f = os.path.join(basedir, expname, 'args.txt')
    with open(f, 'w') as file:
        for arg in sorted(vars(args)):
            attr = getattr(args, arg)
            file.write('{} = {}\n'.format(arg, attr))
    # copy config.txt and save in working directory
